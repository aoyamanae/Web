<!--親のpage-pagge.phpをコピーしてfront-page用に-->
<?php get_header(); ?>
<style>
...
/* div#parentslider{*/
  ...
  z-index: 0; /* 背面に来るように最小値指定 */
  overflow: hidden; /* ボックスからはみ出した部分を表示しない */
  height: 310px;
  /* width:100%;  */
  top: 213px; /* 座標指定 */
  left:0;
} 
#main{ /* NEWSが下に潜り込まないように追加 */
  margin-top: 290px; /*325px*/
}
.widget {
  position: relative; /* ウィジェットが下に潜り込まないように */
  z-index: 2; /* relativeで固定位置指定、z-indexは数字が大きいほど前面 */
}
</style>

  <?php //get_template_part('breadcrumbs-page'); 固定ページパンくずリスト?>
  <?php echo do_shortcode('[metaslider id="49"]'); ?> <!--追加-->
  <?php
